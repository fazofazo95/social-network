  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Feed load failed, falling back to user posts:",
          feedError,
        );
      ...
      >
        ...
        <ul className="flex gap-2 border-t border-purple-500/20 pt-2">
          ...
          <li className="flex ml-auto">
            <button
              type="submit"
              className="flex items-center gap-2 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded-md px-4 py-1.5 transition cursor-pointer disabled:opacity-50 font-semibold"
              disabled={isSubmitting}
            >
        ...
        {postPrivacy === "custom" ? (
          <div className="mt-2 border border-purple-500/20 rounded-md p-2">
            <p className="text-sm text-purple-400 mb-2">
              Choose users who can see this post:
            </p>
            {selectiveUsers.length === 0 ? (
              <p className="text-sm text-purple-400/50">
                No followers/following available to select.
              </p>
            ) : (
