...
type ChatRepository interface {
	...
	GetChatParticipants(ctx context.Context, chatID int) ([]int, error)
}
...
func (r *sqliteChatRepo) SendDirectMessage(ctx context.Context, senderID, targetUserID int, in models.SendMessageInput) (*models.ChatMessage, error) {
	...
	chatID, err := r.ensureDirectChatTx(ctx, tx, senderID, targetUserID)
	if err != nil {
		return nil, err
	}

	msg, err := r.sendMessageToChatTx(ctx, tx, chatID, senderID, in)
	if err != nil {
		return nil, err
	}
...
func (r *sqliteChatRepo) SendGroupMessage(ctx context.Context, senderID, groupID int, in models.SendMessageInput) (*models.ChatMessage, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
...
func (r *sqliteChatRepo) ListChats(ctx context.Context, userID, limit, offset int) ([]*models.ChatSummary, error) {
...
func (r *sqliteChatRepo) GetChatMessages(ctx context.Context, userID, chatID, beforeID, limit int) ([]*models.ChatMessage, error) {
	if limit <= 0 || limit > 100 {
		limit = 30
	}
	...
	if err := r.ensureUserCanAccessChatTx(ctx, tx, userID, chatID); err != nil {
		return nil, err
	}
...
func (r *sqliteChatRepo) MarkChatRead(ctx context.Context, userID, chatID, lastMessageID int) error {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (r *sqliteChatRepo) GetChatParticipants(ctx context.Context, chatID int) ([]int, error) {
	query := `
        SELECT user_id 
...
func (r *sqliteChatRepo) normalizePair(a, b int) (int, int) {
	if a < b {
		return a, b
	}
	return b, a
...
func (r *sqliteChatRepo) validateMessageInput(in models.SendMessageInput) (models.SendMessageInput, error) {
	...
	case "text":
		if in.Body == "" {
			return models.SendMessageInput{}, ErrInvalidChatMessage
		}
	case "image":
		if in.MediaURL == "" {
			return models.SendMessageInput{}, ErrInvalidChatMessage
...
func (r *sqliteChatRepo) canUsersDirectChat(ctx context.Context, tx *sql.Tx, userA, userB int) (bool, error) {
...
func (r *sqliteChatRepo) ensureDirectChatTx(ctx context.Context, tx *sql.Tx, userID, targetUserID int) (int, error) {
	if userID == targetUserID {
		return 0, ErrDirectChatNotAllowed
	}

	allowed, err := r.canUsersDirectChat(ctx, tx, userID, targetUserID)
	if err != nil {
		return 0, err
	}
	if !allowed {
	...
	lowID, highID := r.normalizePair(userID, targetUserID)
...
func (r *sqliteChatRepo) ensureUserCanAccessChatTx(ctx context.Context, tx *sql.Tx, userID, chatID int) error {
	var n int
	if err := tx.QueryRowContext(ctx, `
		SELECT COUNT(*)
		FROM chat_participants
...
func (r *sqliteChatRepo) sendMessageToChatTx(ctx context.Context, tx *sql.Tx, chatID, senderID int, in models.SendMessageInput) (*models.ChatMessage, error) {
	in, err := r.validateMessageInput(in)
	if err != nil {
		return nil, err
	}
	...
	res, err := tx.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?)
	`, chatID, senderID, in.MessageType, r.nullableString(in.Body), r.nullableString(in.MediaURL))
	if err != nil {
		return nil, err
	}
...
func (r *sqliteChatRepo) nullableString(s string) interface{} {
